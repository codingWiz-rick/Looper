...
/**
 * The Looper class provides a convenient way to run a looping execution of a LooperRunnable
 * based on the lifecycle events of a LifecycleOwner.
 */
public class Looper implements LifecycleEventObserver {
    ...
    /**
     * Constructor for the Looper class.
     *
    ...
    /**
     * Factory method to create a Looper instance.
     *
     * @param s The name of the Looper instance.
     * @return A new Looper instance.
     */
    @NonNull
    @Contract("_ -> new")
    public static Looper getInstance(String s) {
        return new Looper(s);
    }

    /**
     * Posts a LooperRunnable to be executed in a loop.
     *
     * @param runnable The LooperRunnable to execute.
     */
    public void post(LooperRunnable runnable) {
    ...
    /**
     * Set the time interval between executions of the LooperRunnable.
     *
    ...
    public void destroy() {
        if (handlerThread.isAlive()) {
            ...
            Log.i(TAG, "Looper: Instance Already Destroyed");
        }
    }
    ...
    public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event) {
        ...
        switch (event) {
            case ON_CREATE:
                start();
                Log.d(TAG, "Looper Thread: " + handlerThread.getName() + " onCreate: called on " + substring + "Class");
                Log.d(TAG, "--------------------------------------------------------------------------------------------");
                break;
            case ON_START:
                start();
                Log.d(TAG, "Looper Thread: " + handlerThread.getName() + " onStart: called on " + substring + "Class");
                Log.d(TAG, "--------------------------------------------------------------------------------------------");
                break;
            case ON_RESUME:
                resume();
                Log.d(TAG, "Looper Thread: " + handlerThread.getName() + " onResume:  called on " + substring + "Class");
                Log.d(TAG, "--------------------------------------------------------------------------------------------");
                break;
            case ON_PAUSE:
                pause();
                Log.d(TAG, "Looper Thread: " + handlerThread.getName() + " onPause:  called on " + substring + "Class");
                Log.d(TAG, "--------------------------------------------------------------------------------------------");
                break;
            case ON_STOP:
                stop();
                Log.d(TAG, "Looper Thread: " + handlerThread.getName() + " onStop:  called on " + substring + "Class");
                Log.d(TAG, "--------------------------------------------------------------------------------------------");
                break;
            case ON_DESTROY:
                Log.d(TAG, "Looper Thread: " + handlerThread.getName() + " onDestroy:  called on " + substring + "Class");
                destroy();
                Log.d(TAG, "--------------------------------------------------------------------------------------------");
                break;
        }
    ...
    static class LoopRunnable implements Runnable {
        ...
        /**
         * Set the LooperRunnable to execute in a loop.
         *
         * @param runnable The LooperRunnable to execute.
         */
